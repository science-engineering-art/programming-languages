...
func (s Server) Listening() {
	...
	for {
		...
		wPort := WritePort{Buffer: buffer, Signal: signal}
		s.Port80 <- wPort

		rPort := ReadPort{Buffer: buffer, Signal: signal}
		wg.Add(1)
		go Receive(ID, rPort)
	}
}
...
func Receive(ID int, port ReadPort) {
...
func main() {
	...
	var s Server
	s.Port80  = make(chan WritePort, 1)
	s.Accept  = make(chan int, 1)
	s.Request = make(chan int, 1)
